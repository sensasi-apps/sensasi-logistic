...
@push('modal')
    <div x-data="crud(materialManufactureCrudConfig)" @@material-manufacture:open-modal.document="openModal"
        ...
        <x-_modal centered>
            ...
            <form method="POST" @@submit.prevent="submitForm" id="{{ uniqid() }}">
                ...
                <ul class="nav nav-tabs text-capitalize" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="material-in-tab" data-toggle="tab" data-target="#material-in"
                            type="button" role="tab" aria-controls="material-in"
                            aria-selected="false">{{ __('material ins') }}</button>
                    </li>
                </ul>
                    ...
                    <div class="tab-pane fade" id="material-in" role="tabpanel" aria-labelledby="material-in-tab">
                                    ...
                                    <div class="list-group-item p-4">
                                        ...
                                        <div class="d-flex justify-content-between align-items-end">
                                            ...
                                            <button type="button" class="btn btn-icon btn-outline-danger" tabindex="-1"
                                                ...
                                                :disabled="detail.out_details?.length > 0"
                                                @@click.prevent="formData.material_in.details.splice($i, 1)">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div class="form-group d-flex justify-content-between">
                            <div>
                                ...
                                <div>
                                    <strong
                                        x-effect="$data.total_out_price = formData.material_in?.details?.reduce((a, b) => a + b.qty * b.price, 0)"
                                    ...
                                    <div>
                                        <a x-init="$($el).tooltip({ boundary: 'window' })" title="{{ __('Auto calculate from material outs') }}"
                                            @@click="formData.material_in?.details.forEach(detail => detail.price = total_in_price / formData.material_in.details.length / (detail.qty || (detail.qty = 1)))"
                                            class="text-warning">
                            ...
                            <div>
                                <a href="javascript:;" @@click="formData.material_in.details.push({})"
                                    class="badge badge-success mr-3 text-capitalize"><i class="fas fa-plus mr-1"></i>
                                    {{ __('add material in') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @slot('footer')
                ...
                <div>
                    <x-_disabled-delete-button
                        x-show="formData.material_in?.details?.find(detail => detail.out_details?.length > 0)"
                        x-init="$($el).tooltip()" :title="__('cannot be deleted. Material(s) has been used')" />
...
@push('js')
    <script>
        ...
        const materialManufactureCrudConfig = {
            blankData: {
                'id': null,
                'code': null,
                'at': null,
            ...
            dispatchEventsAfterSubmit: [
                'material-manufacture:datatable-draw'
            ],
